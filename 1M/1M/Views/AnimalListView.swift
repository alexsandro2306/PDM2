...
struct AnimalListView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            print("🔄 AnimalListView apareceu - Animais no Core Data: \(allAnimals.count)")
            
            // Verificar periodicamente
            Timer.scheduledTimer(withTimeInterval: 2.0, repeats: true) { timer in
                print("CHECK: Animais no Core Data: \(allAnimals.count)")
            ...
            if allAnimals.isEmpty {
                ...
                print("✅ Já existem \(allAnimals.count) animais")
            }
        }
        .refreshable {
    ...
    private func loadAnimalsFromAPI() {
        ...
        animalService.fetchAnimals { result in
        ...
        await withCheckedContinuation { continuation in
            animalService.fetchAnimals { result in
                DispatchQueue.main.async {
                    switch result {
                    case .success(let animals):
                        print("🔄 \(animals.count) animais atualizados")
                    case .failure(let error):
                        errorMessage = error.localizedDescription
                        showingError = true
                    }
...
struct AnimalRowView: View {
    ...
    var body: some View {
        HStack {
            // Placeholder para foto (pode mos adicionar depois)
            ZStack {
                Rectangle()
